...
def find_login_by_session(session):
    ...
    if(is_mac_username(session['User-Name'])):
        ...
        # 1. Ищем mac+vlan
        logins_resp = requests.get(redis_ws + 'raw?query=FT.SEARCH idx:radius:login @mac:{' \
        ...
        logins_resp = requests.get(redis_ws + 'raw?query=FT.SEARCH idx:radius:login @onu_mac:{' \
            + mac_from_hex(session['ADSL-Agent-Remote-Id']).replace(':', '\\:') + '}')
        if(logins_resp.ok):
            logins = json.loads(logins_resp.text)
            if(logins and len(logins) == 1):
                return list(logins.values())[0]
    ...
    # Сессия PPPOE или статика
        ...
        if(m):
            ip = m.groups()[0]
            logins_resp = requests.get(redis_ws + 'raw?query=FT.SEARCH idx:radius:login @ip_addr:{' \
            ...
            if(logins_resp.ok):
                login = json.loads(logins_resp.json())
                return login
